...
[2026-08-03 정정] 이전 버전은 이 경로를 "호출 예산(1,000회) 대비 289배 부족해
전수 인덱싱이 원리상 불가능하다"는 이유로 채택했다고 서술했으나, "1,000회"는
KOSIS 공식 문서에서 확인되지 않은 수치였다(문서 19 §5.2, 출처 표기 없음).
공식 확인된 건 "분당 200회" 속도 제한뿐이며, 이 기준으로는 28만 표 전수 조회가
이론상 하루 안팎에 가능하다 — "호출량 부족으로 불가능"은 아니다.

그래도 경로 C를 채택한 진짜 이유는 다르다: (1) 표를 전부 받아와도 그걸 의미
있게 분류·검색 가능하게 정리하는 작업은 호출량과 무관한 별도의 비용이고,
(2) KOSIS 통합검색이 이미 그 정리(랭킹) 작업을 대신 해주므로 중복 구축할
필요가 없으며, (3) 표는 계속 추가·개정되어 "전수 수집"이 한 번으로 끝나지
않는다. 주장당 1회 호출로 28만 표 전체를 사정권에 두는 실용적 경로다.
