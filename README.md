...
현재 공개 데모는 **Shadow Mode(연구·검토용 모드)** 를 함께 제공합니다. Shadow Mode는 운영 판정이나
발행 결과를 바꾸지 않고, 문장 선택부터 KOSIS 근거 연결·API 스냅샷·사람 검토까지의 과정을 연구 기록으로 남깁니다.
...
## Shadow Mode — 운영 판정을 바꾸지 않는 검증 실험실

Shadow Mode는 “통계표를 찾았다”에서 멈추지 않고, **기사 주장과 실제로 비교 가능한 공식 근거가 준비됐는지** 확인하는
연구·검토 흐름입니다. 이 모드의 저장·검토 결과는 운영 Claim·리뷰·판정 이력을 바꾸지 않습니다.

```mermaid
flowchart LR
    A["검토할 수치 문장 선택"] --> B["KOSIS 후보 Top-K 탐색"]
    B --> C["근거 객체 저장\n표·지표·단위·시간·선택항목"]
    C --> D["공식 KOSIS API\n스냅샷 보존"]
    D --> E["기사 값과 실제 값 대조"]
    E --> F["승인 · 검토 필요 · 반려\n+ JSON/CSV 이력"]
...
1. 검토할 수치 문장을 선택합니다.
2. KOSIS 후보 통계표를 탐색하고, 지표·단위·시간축·선택항목을 포함한 근거 객체를 만듭니다.
3. 공식 API 조회 결과를 스냅샷으로 저장한 뒤, 기사 값과 비교합니다.
4. 검토자는 승인·검토 필요·반려와 메모를 남기고, JSON/CSV로 실행 근거를 내려받습니다.

**안전 원칙:** 기사 발행일의 상대 시점(예: “지난달”), KOSIS 기준 월, 단위, 대상, 선택항목이 모두 확인될 때만
승인 후보가 됩니다. 하나라도 애매하면 자동 승인하지 않고 `검토 필요`로 남깁니다. LLM은 후보·추출 보조이며,
실제 값 비교의 기준은 KOSIS API 스냅샷과 결정적 코드입니다.

## 숫자로 보는 현재 상태

| 항목 | 값 |
|---|---|
| 테스트 | **최신 전체 테스트 통과** (`python -m pytest -q`, 2026-07-30, `live` 제외). 4계층 구조 — 기본(무과금·결정적)·`@dataset`(실물 회귀)·`@contract`(HCX 재생)·`@eval`(정확도 게이트) |
...
| Shadow Mode | 수치 문장 선택 → 후보 탐색 → 근거 객체 → API 스냅샷 → 값 대조·검토 → JSON/CSV 이력. 운영 판정에는 영향을 주지 않는 연구 모드 |
...
- **Shadow Mode로 (연구·검토)** — `검증 실험실 → Shadow Mode`에서 문장을 하나 고른 뒤,
  KOSIS 후보 탐색, 근거 객체 저장, API 스냅샷 준비, 실제 값 대조, 검토 상태 저장 순서로 실행해 보세요.
  기사 시점·KOSIS 월·단위·선택항목을 함께 확인하는 것이 핵심입니다.
- **코드로 (개발자)** — 클론 후 테스트 한 줄(`python -m pytest -q`)이 외부 API·키 없이
